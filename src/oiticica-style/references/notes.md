# Notes for oiticica-style

## Source Basis

Oiticica names six essential qualities and six corresponding defects in §5 of the first part of *Manual de Estilo*. In §25, he says it is easier and more pedagogical to avoid defects before acquiring the qualities jointly. This supports a defect-first aggregate pipeline; it does not support six independent defect topics.

## Eval Polarity

The positive pipeline cases contain an evidenced defect or failed quality for the router to repair or carry through both stages. The negative preservation case contains no target defect and requires the router to preserve a strong passage. The strong description case tests genre routing and complete aggregate coverage without requiring the model to invent a fault. The narrow case tests that an explicit skill request suppresses the general pipeline. Skilpel supplies only the selected router body, so the narrow case does not assert the child skill’s exact rewrite; `oiticica-concision` owns that behavior, and a read-only Codex trace verifies the nested load.

## Source Boundary

Every eval passage is an exact quotation or verbatim excerpt from a public-domain, widely read English work or civic or sacred text. No eval fixture is invented or a source-model paraphrase.

## Defect Repair

- Author: William Shakespeare
- Work: *The Merchant of Venice*
- Location: act 3, scene 2, Antonio’s letter
- Boundary: Verbatim public-domain excerpt. The nonstandard pronoun case supplies the impurity.

### Quotation

between you and I

## Strong Preservation

- Author: Jane Austen
- Work: *Pride and Prejudice*
- Location: chapter 1, opening sentence
- Boundary: Exact public-domain quotation.

### Quotation

It is a truth universally acknowledged, that a single man in possession of a good fortune, must be in want of a wife.

## Obscurity

- Source: Berean Literal Bible
- Work: The Gospel According to John
- Location: 21:15
- Boundary: Verbatim modern public-domain excerpt with its verse number retained.

### Quotation

15 Therefore when they had dined, Jesus says to Simon Peter, “Simon son of John, do you love Me more than these?”

## Description

- Author: Charles Dickens
- Work: *Bleak House*
- Location: chapter 1, opening fog sequence
- Boundary: Exact public-domain quotation.

### Quotation

Fog up the river, where it flows among green aits and meadows; fog down the river, where it rolls defiled among the tiers of shipping.

## Causal Relation Repair

- Author: William Livingston Klein
- Work: *Why We Punctuate; or, Reason Versus Rule in the Use of Marks*
- Location: §49
- Boundary: Exact public-domain illustrative quotation used to show two causal readings distinguished by punctuation.

### Quotation

John did not go to town because his father was absent.

## Stock Description Repair

- Author: Edward Bulwer-Lytton
- Work: *Paul Clifford*
- Location: chapter 1, opening sentence
- Boundary: Verbatim public-domain excerpt. The first clause supplies stock scene-setting; the second supplies the concrete weather needed for revision.

### Quotation

It was a dark and stormy night; the rain fell in torrents

## Ambiguity Repair

- Source: Berean Literal Bible
- Work: The Gospel According to John
- Location: 21:15
- Boundary: Verbatim modern public-domain excerpt with its verse number retained.

### Quotation

15 Therefore when they had dined, Jesus says to Simon Peter, “Simon son of John, do you love Me more than these?”

## Narrow Concision

- Author: Robert Louis Stevenson
- Work: *Strange Case of Dr Jekyll and Mr Hyde*
- Location: Henry Jekyll’s Full Statement of the Case
- Boundary: Exact public-domain quotation.

### Quotation

I made my preparations with the most studious care.
